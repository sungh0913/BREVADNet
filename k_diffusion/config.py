...
from . import layers, utils, models
...
def make_inner_model(config):
    """
    根据配置文件创建并返回底层的去噪网络实例。
    """
    model_config = config['model']
    model_type = model_config.get('type')

    if model_type == 'openai_unet':
        return models.UNetModel(
            image_size=model_config['input_size'],
            in_channels=model_config['in_channels'],
            out_channels=model_config['out_channels'],
            model_channels=model_config['ch'],
            num_res_blocks=model_config['num_res_blocks'],
            attention_resolutions=model_config['attn_resolutions'],
            dropout=model_config.get('dropout', 0.0),
            channel_mult=model_config['ch_mult'],
            conv_resample=True,
            dims=2,
            num_classes=None,
    ...
    if model_config.get('type') == 'fusion_unet':
        return models.FusionUNet(config)
